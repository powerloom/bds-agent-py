...
| **`min_usd`** | **`threshold`** (number) | Alert if any **Swap** in the epoch is ≥ threshold USD (reports largest qualifying). |
...
3. **Remote agent / LLM**: there is no separate schema service yet. The table above + **`tests/test_rules.py`** are the source of truth until **`bds-agent create`** embeds this in prompts (coord doc **`bds-mpp-integration/12-agentic-framework.md`** — LLM compiler / `agent.yaml` plan).
