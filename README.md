...
**Status**: `signup` and `credits` are wired to the [bds-agenthub-billing-metering](https://github.com/powerloom/bds-agenthub-billing-metering) HTTP API. `run`, **`bds-agent query`** (NL → catalog route + params), and **local MCP** (`bds-agent mcp`, stdio) are implemented. Shared **LLM** layer (`bds-agent llm …`) powers **`query`**; **`create`** is still planned. The **`anthropic`** backend implements **only** the Anthropic Messages API; OpenAI- and Ollama-shaped APIs use the **`openai`** and **`ollama`** backends (see **`docs/USER_GUIDE.md`**).
...
- **`BDS_API_ENDPOINTS_CATALOG_JSON`** — path or URL to `endpoints.json` for route catalog (`run` / `query` / `mcp`; `create` when implemented — see guide)
...
- **`BDS_AGENT_LLM_BACKEND`** — `anthropic` / `openai` / `ollama` / … for **`bds-agent llm`** and future **`query` / `create`**
