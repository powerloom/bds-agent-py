...
from bds_agent.positions import position_sell_tokens
...
def test_position_sell_tokens_uses_recorded_balance() -> None:
    ...
    assert position_sell_tokens(pos, 200.0) == 86.0
    assert position_sell_tokens(pos, 50.0) == 50.0
    assert position_sell_tokens(pos, 0.0) == 0.0


def test_position_sell_tokens_legacy_falls_back_to_wallet() -> None:
    pos = {"entry_price": 0.174, "size_usd": 15.0}
    assert position_sell_tokens(pos, 12.5, fallback_size_usd=15.0, fallback_price_usd=0.174) == 12.5
